...
[Roboflow Dataset]
     ↓
PHA 0 — DATA PREPARATION
     ├── 1. Download dataset (Roboflow API)
     ├── 2. Cleaning dataset (scan lỗi, fix bbox, clip tọa độ)
     └── 3. Huấn luyện mô hình (YOLOv8x / YOLOv8x-pose)
     ↓
[Input Video] + [Model Weights]
     ↓
PHA 1 — TRACKING
     ├── 1. Đọc video → frames
...
[Output Video] [Heatmaps] [Thống kê]
     ↓
PHA 3 — ANALYSIS
     ├── 14. EDA dataset + kiểm tra chất lượng
     ├── 15. Trực quan hóa pipeline component (perspective, optical flow, ...)
     └── 16. Thống kê cầu thủ & biểu đồ huấn luyện
```
...
| Matplotlib | ≥ 3.7.0 | Vẽ biểu đồ thống kê, trực quan hóa dữ liệu |
| Seaborn | ≥ 0.13.0 | Biểu đồ thống kê nâng cao (EDA) |
| PyYAML | ≥ 6.0 | Đọc file cấu hình dataset YAML |
| python-dotenv | ≥ 1.0.0 | Load biến môi trường từ .env file |
| Roboflow | ≥ 1.1.24 | Download dataset từ Roboflow |
